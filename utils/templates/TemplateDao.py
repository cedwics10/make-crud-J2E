...
class TemplateDao(Template):
    ...
    def __init__(self, table_name, table_details):
        ...
        self.output_folder = "dao"
    
    def set_custom_parameters(self):
        column_list = [column_name for column_name in self.columns.keys()]
        update_columns_setters = [column_name + " = ?" for column_name in self.columns.keys()]

        update_columns_binding = "," . join(["?" for column_name in self.columns.keys()])
